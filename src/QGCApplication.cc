    ...
    , _runningUnitTests     (unitTesting)
{
    ...
    QString applicationName = "Vaagai GCS";
    if (_runningUnitTests) {
        // We don't want unit tests to use the same QSettings space as the normal app. So we tweak the app
        // name. Also we want to run unit tests with clean settings every time.
        applicationName = QStringLiteral("Vaagai GCS"); //QStringLiteral("%1_unittest").arg(QGC_APPLICATION_NAME);
    } else {
#ifdef DAILY_BUILD
        // This gives daily builds their own separate settings space. Allowing you to use daily and stable builds
        // side by side without daily screwing up your stable settings.
        //applicationName = QStringLiteral("%1 Daily").arg(QGC_APPLICATION_NAME);
        applicationName = QStringLiteral("Vaagai GCS");
#else
        applicationName = QStringLiteral("Vaagai GCS"); //QGC_APPLICATION_NAME;
#endif
    }
